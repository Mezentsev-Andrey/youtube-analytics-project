...
class PlayList:
    """
    Класс для получения данных плейлиста из YouTube-канала.
    """
    ...
    def get_video_id(self) -> list[str]:
        """
        Возвращает список видео по id из плейлиста.
        """
        playlist_videos = (
            self.get_service().playlistItems().list(playlistId=self.id_playlist, part="contentDetails").execute()
        )
        return [video["contentDetails"]["videoId"] for video in playlist_videos["items"]]

    def get_title_playlist(self) -> Any:
        """
        Возвращает title плейлиста для инициализации.
        """
        pl_info = self.get_service().playlists().list(id=self.id_playlist, part="snippet").execute()
    ...
    def total_duration(self) -> datetime.timedelta:
        """
        Возвращает объект класса с суммарной длительностью плейлиста.
        """
        video_response = (
            self.get_service()
            .videos()
        ...
        td_sum = datetime.timedelta(seconds=sum(td.total_seconds() for td in total_duration_list))
        return td_sum

    def show_best_video(self) -> Any:
        """
